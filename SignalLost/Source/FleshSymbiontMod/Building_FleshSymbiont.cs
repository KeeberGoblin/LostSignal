    ...
    public class Building_FleshSymbiont : Building
    {
        ...
        public override void Tick()
        {
            ...
            if (ticksSinceLastBond > (HUNGER_INCREASE_INTERVAL / FleshSymbiontSettings.hungerGrowthMultiplier))
            {
                ...
                if (hungerLevel >= 2 && FleshSymbiontSettings.enableHorrorMessages)
                {
                    ...
                    {
                        3 => "The flesh symbiont writhes in ravenous hunger, its alien whispers growing into screams that claw at your colonists' minds...",
                        2 => "The flesh symbiont pulses hungrily, its whispers growing stronger...",
                        1 => "The flesh symbiont seems restless.",
                        _ => ""
                    };
        ...
        public void OnPawnBonded(Pawn pawn)
        {
            if (!bondedPawns.Contains(pawn))
            {
                ...
                {
                    var compel = AbilityDef.Named("SymbiontCompel");
                    var heal = AbilityDef.Named("SymbiontHeal");
                    var rage = AbilityDef.Named("SymbiontRage");
                ...
                if (FleshSymbiontSettings.enableHorrorMessages)
                {
                    string message = FleshSymbiontSettings.messageFrequency switch
                    {
                        3 => $"The flesh symbiont's tendrils pierce deep into {pawn.Name.ToStringShort}'s spine with a wet, tearing sound. " +
        ...
        public override string GetInspectString()
        {
            ...
            string hungerDesc = hungerLevel switch
            {
                0 => "Dormant",
                1 => "Restless",
                2 => "Hungry", 
