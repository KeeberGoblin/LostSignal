...
namespace FleshSymbiontMod
{
    ...
    public class IncidentWorker_FleshSymbiontDiscovery : IncidentWorker
    {
        ...
        private string GetDiscoveryLetterText()
        {
            ...
            {
                3 => "CLASSIFIED FIELD REPORT - PRIORITY ALPHA\n\n" +
                ...
                2 => "Your colonists have discovered a pulsing, organic mass buried in the ground. The ribbed, hand-like appendage seems to twitch with its own life force.\n\n" +
                ...
                1 => "Your colonists have discovered a strange organic artifact. Some of them seem drawn to it.",
        ...
        protected override float AdjustedChance
        {
            get
            {
                float chance = base.AdjustedChance;
                
                // Reduce chance if map already has symbionts and limiting is enabled
                if (FleshSymbiontSettings.limitSymbiontsPerMap)
                {
                    var map = Find.CurrentMap;
                    if (map != null)
                    {
                        var existingCount = map.listerBuildings.AllBuildingsColonistOfClass<Building_FleshSymbiont>().Count();
                        if (existingCount > 0)
                        {
                            chance *= 0.3f; // Significantly reduce chance for additional symbionts
                        }
                    }
                }
